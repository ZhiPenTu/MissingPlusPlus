...
**平台范围**：**Apple only** —— macOS（主目标）+ iOS（v1 简化单页）。Windows / Linux / Android 已删除。
...
- **Tauri 2.x**（macOS + iOS targets）+ plugins（notification / store / dialog / fs / global-shortcut / os）
...
| 持久化 | **A JSON file**（macOS: `~/Library/Application Support/com.tuzhipeng.MissingPlusPlus/records.json`） |
...
│   │   ├── commands/{records,notifications,storage,updater}.rs
│   │   ├── frontend/{manifest,downloader,installer}.rs  # C 架构 updater
│   │   ├── platform/macos.rs                             # menu bar + ⌥M
│   │   └── error.rs
│   └── tauri.conf.json                                   # bundle = app + dmg
├── src/                # React frontend
...
# produces Missing++_0.1.0_x64.app  +  Missing++_0.1.0_x64.dmg
```

Bundle targets: `app` + `dmg`（Apple-only，no Windows MSI / Linux AppImage）。
