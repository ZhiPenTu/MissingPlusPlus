...
25. [Update Checker (v0.0.2+)](#25-update-checker-v002)
26. [Update Checker v0.0.11+ 增量](#26-update-checker--v0011-增量-in-app-download--安装流程--keychain-修复)
...
## 26. Update Checker — v0.0.11+ 增量 (in-app download + 安装流程 + keychain 修复)

§25 描述 v0.0.2 的检测 (banner + 跳转 GitHub)。v0.0.11 起扩展为 in-app
download + progress + 立即安装,后续 4 个版本(v0.0.21-25)修复 5 个 keychain
/ install 流程问题。这一节是 source of truth,改之前先核对。

### 关键文件
- `Services/UpdateDownloader.swift` — `URLSessionDownloadTask` + `NSSavePanel`,
  `onProgress` / `onComplete(localURL)` / `onError` 回调
- `Services/UpdateInstaller.swift` — detached `nohup /usr/bin/open` +
  1.5s 后 `exit(0)`,释放 `/Applications/.app` 锁
- `Views/UpdateBanner.swift` — 4 状态 banner: `.available` / `.downloading`
  / `.downloaded` / `.installing`
- `Views/MenuBarContent.swift` — `onInstall` closure 切到 `.installing`
  状态,不再 `withAnimation { updateState = nil }`
- `Services/KeychainService.swift` — `GetResult` enum 区分 `.found /
  .notFound / .locked / .other`,不再静默吞
- `Services/AppPreferences.swift` — `aiAPIKey` 内存缓存 + lazy load
  + `hasAIKey` flag
- `MissingPlusPlusTests/UpdateDownloaderTests.swift` — 5 cases (task
  description round-trip via `.path`、filename pattern、state
  Equatable 等)
- `MissingPlusPlusTests/UpdateInstallerTests.swift` — 6 cases
  (detachedOpenScript shell escape 各 path 形态)
- `MissingPlusPlusTests/AppPreferencesKeychainTests.swift` — 7 cases
  (GetResult 各种状态、migration、stale flag 修正)

### v0.0.21+ 修复的不变量 (P0/P1 bugs)

#### P0-1: NSSavePanel 避免 quarantine auth
- **不要** 用 `URLSessionDownloadTask` 直接下到 `temporaryDirectory`。
  sandbox 容器路径下文件会被打 `com.apple.quarantine`,后续
  `NSWorkspace.open` 触发"钥匙串验证" auth dialog。
- **要** 用 `NSSavePanel` 让用户选保存位置 (推荐 `~/Downloads`),
  那里没 quarantine 属性,直接打开无 auth。
- API: `UpdateDownloader.shared.startDownload(assetURL:suggestedFilename:)`
  → 弹 panel → 用户选 → `beginDownload(from:to:)` → 走
  `URLSessionDownloadTask`。

#### P0-2: destURL.path (NOT .absoluteString) 进 taskDescription
- URL.absoluteString 把 `:` URL-encode 成 `%3A`,taskDescription
  存的是 `"file%3A///Users/foo/x.dmg"`。nonisolated delegate 调
  `URL(fileURLWithPath: destPath)` 当字面路径读,DMG 写到
  `file%3A/...` 怪位置,不在用户选的地方。
- **永远** 用 `destURL.path` (未编码 filesystem path)。
- 测试 `test_taskDescription_roundTripViaPath` pin 住这个 contract。

#### P0-3: UpdateInstaller 检查 shell exit status
- `/bin/sh -c "nohup /usr/bin/open ..."` 跑完, `task.terminationStatus != 0`
  表示 shell 失败 (e.g. 路径 escape 错, `/usr/bin/open` 不存在)。
- **要** 检查 `terminationStatus`,非零时 NSLog + fallback 到
  `NSWorkspace.shared.open` + **不要** `exit(0)`。让用户手动关 app 再拖。
- 脚本构造提取到 `UpdateInstaller.detachedOpenScript(forDMG:)` 做
  static func,可单测 single-quote escape。

#### P0-4: 1.5s 后 `exit(0)` (NOT `NSApp.terminate(nil)`)
- `NSApp.terminate` 走 graceful 路径 (shouldTerminate → willTerminate
  → runloop flag → exit),0.3s 不足以保证 process 真死,用户拖 .app
  时还会被"in use" dialog 卡。
- `exit(0)` 直接 _exit,无回调、无 runloop,立刻释放 fd 锁。
- 安全前提: UserDefaults Foundation auto-flush, missings.json
  MissingStore.add 同步写, no pending resources。
- 1.5s 是经验值:detached open 启动 + 用户反应时间。

#### P0-5: KeychainService.get returns GetResult enum
- 旧 `String?` 把 `.notFound` 和 `.locked` (errSecInteractionNotAllowed)
  都当 nil 静默吞,AppPreferences 看不到信号。
- 新 `GetResult` enum: `.found(String) / .notFound / .locked / .other(OSStatus)`。
- `errSecItemNotFound` → notFound: 可清 stale flag
- `errSecInteractionNotAllowed` → locked: 不动 flag,下次重试

### v0.0.24+ keychain lazy load + hasAIKey

- `AppPreferences.init` **不** 读 keychain (避免 login keychain locked 时
  弹"登录钥匙串密码" dialog)。
- `aiAPIKey` getter 是 lazy load:第一次访问触发 keychain 读,后续走
  `_cachedAIKey` 缓存。setter 写 keychain 失败时回滚 in-memory + flag。
- `hasAIKey: Bool` flag 存 UserDefaults (只 flag, 不存 key 本身)。
  `aiIsConfigured` 用 flag 答"是否已配",**不碰 keychain**。
- Migration: 旧版没 flag → lazy load 看到 .found → flag 修 true。
  看到 .notFound (用户从 Keychain Access.app 手动删了) → flag 修 false。
  看到 .locked → 不动 flag,下次重试。

### 4 状态 banner 状态机
```
.available      → "下载" 按钮 → 弹 NSSavePanel
.downloading    → 进度条 + "稍后" (cancel download)
.downloaded     → "立即安装" → 切到 .installing → 1.5s 后 exit(0)
.installing     → spinner "正在准备安装…" (不可取消,exit 前)
```

### 不要做 (in-app download / install 流程)
- **不要** 把下载路径写回 `temporaryDirectory` (P0-1 quarantine bug)
- **不要** 用 `destURL.absoluteString` (P0-2 URL-encoding bug)
- **不要** 用 `NSApp.terminate(nil)` 代替 `exit(0)` (P0-4 1.5s 不够)
- **不要** 在 `AppPreferences.init` 读 keychain (P0-5 login keychain dialog)
- **不要** 跳过 `task.terminationStatus` 检查 (P0-3 静默失败)
- **不要** 在 `onInstall` closure 里 `withAnimation { updateState = nil }` —
  会让 banner 在 exit(0) 前 1.5s 消失,用户可能重复点
  (切 `.installing` 状态保留 banner)
- **不要** 在 NSSavePanel 还在弹时再点"下载" — `isStartingDownload` flag
  已经守住了,别去掉
- **不要** 把 `detachedOpenScript` 重新 inline 到 `openDMG` —
  提出来就是为单测 shell escape
